# RAM Framebuffer Analog Clock on a ST7789 LCD
...
The objective of this project was to draw a live analog clock face — hour, minute, and second hands — on a Waveshare 1.69-inch, 240 × 280 ST7789 Touch LCD, driven by a DigiComp ESP32-S3 over SPI using MicroPython.
...
| Framebuffer | MicroPython `framebuf.FrameBuffer` |
| Time Source | ESP32-S3 onboard RTC |
...
The ESP32-S3's onboard `RTC` object supplies hour, minute, and second each loop:
...
The RTC can be set once (e.g. `rtc.datetime((2026, 9, 19, 5, 13, 50, 0, 0))`) if it doesn't already hold the correct time.

## 9. Sending the Frame to the LCD
...
The display window is set to the full 240 × 280 screen once, then the entire RAM buffer is streamed out in a single `spi.write()` call.

## 10. Main Loop

The main loop polls the RTC and only redraws and re-sends a frame when the second value actually changes, avoiding redundant SPI transfers:
...
while True:
    ...
    if second != last_second:
        last_second = second
        draw_clock(hour, minute, second)   # draw into RAM
        show()                              # push RAM to LCD
...
The DigiComp ESP32-S3 successfully renders a smooth, flicker-free analog clock on the Waveshare ST7789 LCD, with the face, 60 minute markers, and three moving hands all composited in RAM each second and pushed to the display in one high-speed SPI transfer.
